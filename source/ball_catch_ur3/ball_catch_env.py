...
from .ball_catch_env_cfg import BallCatchUR3EnvCfg


class BallCatchUR3Env(DirectRLEnv):
    cfg: BallCatchUR3EnvCfg

    def __init__(self, cfg: BallCatchUR3EnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # Tensor used for the actions
        self.robot_dof_targets = torch.zeros((self.num_envs, self.robot.num_joints), device=self.device)
        self.robot_dof_targets[:,:] = self.robot.data.default_joint_pos
        self.robot_dof_lower_limits = self.robot.data.soft_joint_pos_limits[0, :, 0].to(device=self.device)
        self.robot_dof_upper_limits = self.robot.data.soft_joint_pos_limits[0, :, 1].to(device=self.device)
        self.robot_dof_max_joint_vel= self.robot.data.soft_joint_vel_limits[:, self._arm_dof_idx].to(device=self.device)
        # integration time step for the joints actions
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        # clamp the neural network head output, normalize, 
        normalized_actions = (actions.clone().clamp(-3.0, 3.0)/3)
        # la j'ai definit une vitesse max de 2.0 rad/s pour tous les joints, mais tu utiliser aussi robot.data....joint_limits..., je me rappelle 
        # pas trop la methode de la classe Articulation, (à avoir dans ta version isaac lab)
        max_vel = 2.0
        delta_pos = normalized_actions * max_vel * self.dt
        self.actions = delta_pos
        targets = self.robot_dof_targets + delta_pos
        # clamp the target joint positions into the joint limits
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # add variable for failure detection
        self.terminated_failure = hit_arm | ball_on_ground
        # othe variables
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        # radomize initial joint positions for each joint
        joint_pos += sample_uniform(*self.cfg.joint_pos_random_range, joint_pos.shape, joint_pos.device) # type: ignore
        # set initial joint positions command
        self.robot_dof_targets[env_ids] = joint_pos
        # set joint position to sim
        ...
        # Randomize ball initial state
        ball_state = self.ball.data.default_root_state[env_ids].clone()

        ball_state[:, 0] = sample_uniform(*self.cfg.ball_spawn_x_range, len(env_ids), self.device) # type: ignore
        ball_state[:, 1] = sample_uniform(*self.cfg.ball_spawn_y_range, len(env_ids), self.device) # type: ignore
        ball_state[:, 2] = sample_uniform(*self.cfg.ball_spawn_z_range, len(env_ids), self.device) # type: ignore
        # La façon que toi ajouter le bruit à la position de la balle est pas correct
        # le reset state ball doit fait sans le bruit, pour ajouter du bruit sur la postion tu doit ajouter dans la fonction get_obs
        if self.cfg.enable_ball_position_noise:
            ball_state[:, 0:3] += torch.randn((len(env_ids), 3), device=self.device) * self.cfg.ball_position_noise_std # type: ignore
...
def compute_rewards(
    ...
    rew_action = -0.01 * torch.sum(actions ** 2, dim=-1)
    rew_pass = 300.0 * passed.to(dtype=torch.float32)
    # j'ai ajouter une nouvelle varialble pour la récompense de la terminaison "self.terminated_failure"
    # j'ai fait ça parce que la variable "reset_terminated" que tu utilisais avant donnais une mauvaise recompense meme si le robot avait attraper la balle
    rew_termination = -100.0 * reset_terminated.to(dtype=torch.float32)
    # utiliser juste la récompense pour faillure et réussite
    return rew_termination+rew_pass
